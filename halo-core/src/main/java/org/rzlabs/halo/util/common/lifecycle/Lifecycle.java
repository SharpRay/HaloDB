package org.rzlabs.halo.util.common.lifecycle;

import com.google.common.base.Preconditions;
import com.google.common.collect.Lists;
import com.sun.tools.javadoc.Start;
import org.apache.commons.lang3.StringUtils;
import org.rzlabs.halo.util.common.ISE;
 ...
 *    JettyServerModule, but any sort of 'server' that excepts most (or some specific) Lifecycle objects to be
 *    initialized by the time it starts, and still available at the time it stops can logically live in this stage.
...
public class Lifecycle {
    ...
    public Lifecycle(String name) {
        Preconditions.checkArgument(StringUtils.isNotEmpty(name),
                "Lifecycle name must not be null or empty");
        this.name = name;
     ...
     * at Stage.NORMAL. If the lifecycle has already been started, it throws an {@link ISE}
     *
     ...
     *
     * @param o     The object to add to the lifecycle
     * @param stage The stage to add to lifecycle at
     *
     * @throws ISE {@link Lifecycle#addHandler(Handler, Stage)}
     */
    ...
    /**
     * Adds an instance with a start() and/or close() method to the Lifecycle at Stage.NORMAL. If the lifecycle
     * has already been started, it throws an {@link ISE}
     *
     * @param o The object to add to the lifecycle
     *
     * @throws ISE {@link Lifecycle#addHandler(Handler, Stage)}
    ...
    /**
     * Adds an instance with a start() and/or close() method to the Lifecycle. If the lifecycle has already
     * been started, it throws an {@link ISE}
     *
     * @param o     The object to add to the lifecycle
    ...
    /**
     * Adds a handler to the Lifecycle at the Stage.NORMAL stage. If the lifecycle has already been started, it
     * throws an {@link ISE}
     *
     * @param handler The handler to add to the lifecycle
     *
     * @throws ISE {@link Lifecycle#addHandler(Handler, Stage)}
     */
    ...
    public void addHandler(Handler handler, Stage stage) {
        ...
        try {
            if (!state.get().equals(State.NOT_STARTED)) {
                throw new ISE("Cannot add a handler after the Lifecycle has started, " +
    ...
    /**
     * Adds a "managed" instance (annotated with {@link LifecycleStart} and {@link LifecycleStop}) to the Lifecycle
     * at Stage.NORMAL and starts it if the lifecycle has alread been started.
     *
     * @param o The object to add to the lifecycle
     *
     ...
     * @param stage The stage to add the lifecycle at
     *
     * @throws Exception {@link Lifecycle#addHandler(Handler, Stage)}
     */
     ...
     * lifecyle has already been started.
     *
     * @param o The object to add to the lifecycle
     *
     * @throws Exception {@link Lifecycle#addMaybeStartHandler(Handler, Stage)}
     ...
     *
     * @param o     The object to add to the lifecycle.
     * @param stage The stage to add the lifecyle at
     *
     * @throws Exception {@link Lifecycle#addHandler(Handler, Stage)}
     */
    ...
    /**
     * Adds a Closeable instance to the lifecycle at {@link Stage#NORMAL} stage, doesn't try to call any "start"
     * method on it, use {@link #addStartCloseInstance(Object)} instead if you need th latter behaviour.
     */
    ...
    public void addMaybeStartHandler(Handler handler, Stage stage) throws Exception {
        if (!startStopLock.tryLock()) {
            // (*) This check is why the state should be changed before startStopLock.lock() in stop(). This check
            // allows to spot wrong use of Lifecycle instead of entering deadlock.
        ...
        try {
            if (state.get().equals(State.STOP)) {
                throw new ISE("Cannot add a handler after the Lifecyel has stopped");
            }
    ...
    public void stop() {
        // This CAS outside of a block guarded by startStopLock is the only reason why state is AtomicReference
        // rather than a simple variable. State change before startStopLock.lock() is needed for the new state
        // visibility during the check in addMaybeStartHandler() marked by (*).
        ...
        try {
            ...
            for (Stage stage : handlers.navigableKeySet().descendingSet()) {
                log.info("Stopping lifecycle [%s] stage [%s]", name, stage.name());
                for (Handler handler : Lists.reverse(handlers.get(stage))) {
                    try {
                        handler.stop();
                    } catch (Exception e) {
    ...
    private static class StartCloseHandler implements Handler {
        ...
        public void stop() {
            ...
            try {
                ...
                log.error(e, "Unable to invoke stop() method on object[%s]", o);
            }
        }
    }
